...
const VAULT = join(DATA, "vault.json");
const BOOKS = join(DATA, "books");
...
type BookRow = { user_id: string; payload: unknown };
...
export async function readVault(): Promise<VaultFile | null> {
  try {
    const raw = JSON.parse(await readFile(VAULT, "utf8")) as VaultFile;
...
export async function snapshotVault(sql: Sql): Promise<void> {
  try {
    const users = await sql<Record<string, unknown>>`select * from "user"`;
...
export async function restoreVault(sql: Sql): Promise<boolean> {
  ...
  for (const a of vault.accounts) {
    try {
      await sql.query(
        `insert into "account" (id, "accountId", "providerId", "userId", password, "accessToken", "refreshToken", "idToken", "accessTokenExpiresAt", "refreshTokenExpiresAt", scope, "createdAt", "updatedAt")
  ...
  for (const p of vault.profiles) {
    try {
      await sql.query(
        `insert into desk_profiles (user_id, username, created_at)
...
export function snapshotSoon(sql: Sql) {
  if (snapTimer) return;
  snapTimer = setTimeout(() => {
    snapTimer = null;
    void snapshotVault(sql);
  }, 2_000);
  if (typeof snapTimer.unref === "function") snapTimer.unref();
}

export async function listFileBooks(): Promise<Array<{ userId: string; raw: string }>> {
  const out: Array<{ userId: string; raw: string }> = [];
  try {
    await mkdir(BOOKS, { recursive: true });
    ...
    for (const name of names) {
      ...
      try {
        ...
        out.push({ userId: name.replace(/\.json$/, ""), raw });
      } catch {
        /* skip */
      }
    }
