...
export const auth = betterAuth({
  ...
  account: {
    ...
    accountLinking: {
      ...
      // Password sign-up is unverified. If we skip this check, anyone can
      // register victim@gmail.com + a password and later inherit the victim's
      // Google login (implicit link). X emails are unique synthetics, so they
      // never collide with Gmail and still work as their own users.
    ...
    user: {
      create: {
        async before(user) {
          ...
          const { HANDLE_EMAIL_DOMAIN } = await import("@/lib/desk/login-handle");
          const { usernameError } = await import("@/lib/desk/profile-server");
          const suffix = `@${HANDLE_EMAIL_DOMAIN}`;
          if (email.endsWith(suffix)) {
            const nick = email.slice(0, -suffix.length);
            if (usernameError(nick)) {
