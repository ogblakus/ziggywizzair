...
//#region node_modules/http_ece/ece.js
var require_ece = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	var keylog;
	if (process.env.ECE_KEYLOG === "1") keylog = function(m, k) {
		console.warn(m + " [" + k.length + "]: " + k.toString("base64url"));
		return k;
	};
	else keylog = function(m, k) {
		return k;
	};
	function decode(b) {
		if (typeof b === "string") return Buffer.from(b, "base64url");
		return b;
	}
	...
	function extractDH(header, mode) {
		...
		} else throw new Error("Unknown mode only " + MODE_ENCRYPT + " and " + MODE_DECRYPT + " supported");
		return {
	...
	function extractSecretAndContext(header, mode) {
		...
		if (header.key) {
			...
			if (result.secret.length !== KEY_LENGTH) throw new Error("An explicit key must be " + KEY_LENGTH + " bytes");
		} else if (header.dh) result = extractDH(header, mode);
		else if (typeof header.keyid !== void 0) result.secret = header.keymap[header.keyid];
		if (!result.secret) throw new Error("Unable to determine key");
	...
	function webpushSecret(header, mode) {
		if (!header.authSecret) throw new Error("No authentication secret for webpush");
	...
	function extractSecret(header, mode, keyLookupCallback) {
		if (keyLookupCallback) {
			if (!isFunction(keyLookupCallback)) throw new Error("Callback is not a function");
		}
		if (header.key) {
			if (header.key.length !== KEY_LENGTH) throw new Error("An explicit key must be " + KEY_LENGTH + " bytes");
			return keylog("secret key", header.key);
		}
		if (!header.privateKey) {
			if (!keyLookupCallback) var key = header.keymap && header.keymap[header.keyid];
			else var key = keyLookupCallback(header.keyid);
			if (!key) throw new Error("No saved key (keyid: \"" + header.keyid + "\")");
			return key;
		}
		return webpushSecret(header, mode);
	}
	function deriveKeyAndNonce(header, mode, lookupKeyCallback) {
		if (!header.salt) throw new Error("must include a salt parameter for " + header.version);
		var keyInfo;
		var nonceInfo;
		var secret;
		...
		} else throw new Error("Unable to set context for mode " + header.version);
	...
	function parseParams(params) {
		...
		if (isNaN(header.rs)) header.rs = 4096;
		var overhead = PAD_SIZE[header.version];
		if (header.version === "aes128gcm") overhead += TAG_LENGTH;
		if (header.rs <= overhead) throw new Error("The rs parameter has to be greater than " + overhead);
		if (params.salt) {
			header.salt = decode(params.salt);
			if (header.salt.length !== KEY_LENGTH) throw new Error("The salt parameter must be " + KEY_LENGTH + " bytes");
		}
		header.keyid = params.keyid;
		if (params.key) header.key = decode(params.key);
		else {
			header.privateKey = params.privateKey;
			if (!header.privateKey) header.keymap = params.keymap;
			if (header.version !== "aes128gcm") header.keylabel = params.keylabel || "P-256";
			if (params.dh) header.dh = decode(params.dh);
		}
		if (params.authSecret) header.authSecret = decode(params.authSecret);
		return header;
	}
	...
	function unpadLegacy(data, version) {
		...
		if (pad + padSize > data.length) throw new Error("padding exceeds block size");
		keylog("padding", data.slice(0, padSize + pad));
		var padCheck = Buffer.alloc(pad);
		padCheck.fill(0);
		if (padCheck.compare(data.slice(padSize, padSize + pad)) !== 0) throw new Error("invalid padding");
	...
	function unpad(data, last) {
		...
		while (i >= 0) {
			if (data[i]) {
				if (last) {
					if (data[i] !== 2) throw new Error("last record needs to start padding with a 2");
				} else if (data[i] !== 1) throw new Error("last record needs to start padding with a 2");
	...
	function decryptRecord(key, counter, buffer, header, last) {
		...
		if (header.version !== "aes128gcm") return unpadLegacy(data, header.version);
	...
	*/
	function decrypt(buffer, params, keyLookupCallback) {
		...
		if (header.version !== "aes128gcm") chunkSize += TAG_LENGTH;
		for (var i = 0; start < buffer.length; ++i) {
			var end = start + chunkSize;
			if (header.version !== "aes128gcm" && end === buffer.length) throw new Error("Truncated payload");
			end = Math.min(end, buffer.length);
			if (end - start <= TAG_LENGTH) throw new Error("Invalid block: too small at " + i);
	...
	function encryptRecord(key, counter, buffer, pad, header, last) {
		...
		if (header.version !== "aes128gcm") {
			...
			if (!last && padding.length + buffer.length < header.rs) throw new Error("Unable to pad to record size");
		} else {
			ciphertext.push(gcm.update(buffer));
		...
		if (tag.length !== TAG_LENGTH) throw new Error("invalid tag generated");
		ciphertext.push(tag);
	...
	function writeHeader(header) {
		...
		if (keyid.length > 255) throw new Error("keyid is too large");
	...
	*/
	function encrypt(buffer, params, keyLookupCallback) {
		if (!Buffer.isBuffer(buffer)) throw new Error("buffer argument must be a Buffer");
		var header = parseParams(params);
		if (!header.salt) header.salt = crypto.randomBytes(KEY_LENGTH);
		var result;
		if (header.version === "aes128gcm") {
			if (header.privateKey && !header.keyid) header.keyid = header.privateKey.getPublicKey();
			result = writeHeader(header);
		} else result = Buffer.alloc(0);
		...
		while (!last) {
			...
			if (header.version !== "aes128gcm") recordPad = Math.min((1 << padSize * 8) - 1, recordPad);
			if (pad > 0 && recordPad === 0) ++recordPad;
			pad -= recordPad;
			var end = start + header.rs - overhead - recordPad;
			if (header.version !== "aes128gcm") last = end > buffer.length;
			else last = end >= buffer.length;
			last = last && pad <= 0;
