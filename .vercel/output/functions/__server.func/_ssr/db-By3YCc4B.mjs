...
//#region node_modules/.nitro/vite/services/ssr/assets/db-By3YCc4B.js
var db_By3YCc4B_exports = /* @__PURE__ */ __exportAll({
	i: () => getSql,
	n: () => ensureDbReady,
	r: () => getPglite,
	t: () => db_exports
...
var PGLITE_DIR = "/workspace/data/pglite";
var PGLITE_LEGACY_DIR = "/tmp/ziggywizzair-pglite";
async function createPgliteSql() {
	globalRef.__pgliteInstance__ ??= (async () => {
		...
		if (!existsSync(join(PGLITE_DIR, "PG_VERSION")) && existsSync(join(PGLITE_LEGACY_DIR, "PG_VERSION"))) cpSync(PGLITE_LEGACY_DIR, PGLITE_DIR, { recursive: true });
		mkdirSync(PGLITE_DIR, { recursive: true });
		const { renameSync } = await import("node:fs");
		const open = async () => {
			rmSync(join(PGLITE_DIR, "postmaster.pid"), { force: true });
			const inst = new PGlite(PGLITE_DIR, { parsers: {
				[OID_INT8]: Number,
				[OID_DATE]: identity,
		...
		const flush = () => {
			pg.syncToFs().catch(() => void 0);
		};
		const timer = setInterval(flush, 15e3);
	...
	const migrate = async () => {
		const migrations = /* #__PURE__ */ Object.assign({
			...
			"/migrations/0003_push.sql": _0003_push_default,
			"/migrations/0004_desk_books.sql": _0004_desk_books_default,
			"/migrations/0005_desk_profiles.sql": _0005_desk_profiles_default,
			"/migrations/0006_desk_password.sql": _0006_desk_password_default
		});
	...
	try {
		const { restoreVault, snapshotVault } = await import("./vault.server-DeQoDthf.mjs");
		if (await restoreVault(sql)) await snapshotVault(sql);
	} catch (err) {
...
//#endregion
export { getSql as i, ensureDbReady as n, getPglite as r, db_By3YCc4B_exports as t };
