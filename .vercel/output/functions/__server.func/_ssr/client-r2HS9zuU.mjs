...
//#region node_modules/.nitro/vite/services/ssr/assets/client-r2HS9zuU.js
...
var BEARER_STAY_KEY = "grok-auth.stay";
...
function getBearerToken() {
	...
	return storageGet(window.localStorage, BEARER_KEY) ?? storageGet(window.sessionStorage, BEARER_KEY);
}
function persistBearer(token, stay = true) {
	if (typeof window === "undefined") return;
	storageSet(window.sessionStorage, BEARER_KEY, token);
	if (stay && token) {
		storageSet(window.localStorage, BEARER_KEY, token);
		storageSet(window.localStorage, BEARER_STAY_KEY, "1");
	} else {
		storageSet(window.localStorage, BEARER_KEY, null);
		storageSet(window.localStorage, BEARER_STAY_KEY, null);
	}
}
function setBearerToken(token) {
	persistBearer(token, token != null && storageGet(window.localStorage, BEARER_STAY_KEY) === "1");
	if (!token) persistBearer(null, false);
}
/**
...
//#endregion
export { signOut as a, signIn as i, client_exports as n, persistBearer as r, authClient as t };
