...
test("ensureAuthSecret reuses a file and does not clobber an env override", () => {
  const root = mkdtempSync(join(tmpdir(), "auth-secret-"));
  const first = ensureAuthSecret(root, {});
  const again = ensureAuthSecret(root, {});
  assert.equal(first, again);
  assert.equal(first.length, 64);
  assert.equal(readFileSync(join(root, AUTH_SECRET_REL_PATH), "utf8").trim(), first);
  assert.equal(ensureAuthSecret(root, { BETTER_AUTH_SECRET: "  from-env  " }), "from-env");
});
